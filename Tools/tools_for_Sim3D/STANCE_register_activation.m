...
% folder as a NIFTI file named ?task.name.
%
% Jason E. Hill
...
V_act = V_MNI;
V_act.fname = [STANCEroot,'/Resource/activations/',task.name,'.nii'];
V_act = spm_create_vol(V_act);
V_act = spm_write_vol(V_act,activationMap);

files = {filename;V_act.fname};
spm_reslice(files, flags);
fn_act_reg = [STANCEroot,'/Resource/activations/r',task.name,'.nii'];
