...
if Ndims == 2
    ...
    switch shape
        ...
        case {'atlas','parcellation'}
            ...
            switch filebase
                case 'aal'
                    filename = [STANCEroot,'/Resource/MNI/aal.nii'];
                ...
                case 'brodmann'
                    filename = [STANCEroot,'/Resource/MNI/brodmann.nii'];
                    if ~exist(filename,'file')
                        filename = [filename,'.gz'];
                    end                    
                    [~,Y_atlas] = STANCE_load_volume(filename);
                ...
                case 'HarvardOxford'
                    if signFlag > 0 % cortical labels
                          if threshold == 0
                              filename = [STANCEroot,'/Resource/MNI/HarvardOxford-cort-maxprob-thr0-1mm.nii'];
                              if ~exist(filename,'file')
                                 filename = [filename,'.gz'];
                              end     
                          elseif threshold == 25 || threshold == 0.25
                              filename = [STANCEroot,'/Resource/MNI/HarvardOxford-cort-maxprob-thr25-1mm.nii'];
                              if ~exist(filename,'file')
                                 filename = [filename,'.gz'];
                              end
                          elseif threshold == 50 || threshold == 0.5
                              filename = [STANCEroot,'/Resource/MNI/HarvardOxford-cort-maxprob-thr50-1mm.nii'];                     
                              if ~exist(filename,'file')
                                 filename = [filename,'.gz'];
                              end
                          else
                          ...
                          if threshold == 0
                              filename = [STANCEroot,'/Resource/MNI/HarvardOxford-sub-maxprob-thr0-1mm.nii.gz'];
                          elseif threshold == 25 || threshold == 0.25
                              filename = [STANCEroot,'/Resource/MNI/HarvardOxford-sub-maxprob-thr25-1mm.nii.gz'];
                          elseif threshold == 50 || threshold == 0.5
                              filename = [STANCEroot,'/Resource/MNI/HarvardOxford-sub-maxprob-thr50-1mm.nii.gz'];                     
                          else
                              error('Unsupported probability threshold')
                          end                          
                    end
                ...
                case 'Craddock'
                    if threshold == 200
                        filename = [STANCEroot,'/Resource/MNI/CC200ROI_tcorr05_2level_all.nii'];
                        if ~exist(filename,'file')
                            filename = [filename,'.gz'];
                        end
                    elseif threshold == 400
                        filename = [STANCEroot,'/Resource/MNI/CC400ROI_tcorr05_2level_all.nii'];
                        if ~exist(filename,'file')
                            filename = [filename,'.gz'];
                        end                        
                    else
                ...
                case 'Brainnetome'
                    if threshold == 25
                        filename = [STANCEroot,'/Resource/MNI/BrainnetomeAtlas_BNA_MPM_thr25_1.25mm.nii'];
                        if ~exist(filename,'file')
                            filename = [filename,'.gz'];
                        end                      
                    else
